...
async def get_msg(client, sender, edit_id, msg_link, i):
    ...
    if ('t.me/c/' in msg_link) or ('t.me/b/' in msg_link):
        ...
        file = ""
        try:
            tech_AP_userbot = Client("saverestricted", session_string=SESSION_STRING, api_hash=API_HASH, api_id=API_ID)
            await tech_AP_userbot.start()
            msg = await tech_AP_userbot.get_messages(chat, msg_id)
            if not msg.media:
                if msg.text:
            ...
            file = await tech_AP_userbot.download_media(
                msg,
            ...
            if msg.media==MessageMediaType.VIDEO_NOTE:
                ...
                try:
                    thumb_path = await tech_AP_userbot.download_media(msg.video_note.thumbs[0].file_id)
                except Exception:
                    thumb_path = None
                ...
                print(f'd: {duration}, w: {width}, h:{height}')
                try:
                    thumb_path = await tech_AP_userbot.download_media(msg.video.thumbs[0].file_id)
                except Exception:
                    thumb_path = None
                ...
                try:
                    thumb_path = await tech_AP_userbot.download_media(msg.document.thumbs[0].file_id)
                except Exception:
                    thumb_path = None

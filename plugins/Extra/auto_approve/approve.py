...
async def auto_approve(client, message: ChatJoinRequest):
    if message.chat.id == AUTH_CHANNEL and join_db().isActive():
        ...
        if data.split("-", 1)[0] == "AP":
            user_id = int(data.split("-", 1)[1])
            AP = await referal_add_user(user_id, message.from_user.id)
            if AP and PREMIUM_AND_REFERAL_MODE == True:
        ...
        if data.startswith("sendfiles"):
            ...
            for file in files:
                ...
                if STREAM_MODE == True:
                    button = [[
                        InlineKeyboardButton('Download / Watch Online', callback_data=f'generate_stream_link:{file_id}') 
                    ]]
                else:
                    button = [[
                        ...
                        InlineKeyboardButton("𝗕𝗢𝗧 𝗢𝗪𝗡𝗘𝗥", url="t.me/Dankdev")
                    ]]
                ...
                filesarr.append(msg)
            k = await client.send_message(chat_id = message.from_user.id, text=f"This Movie Files/Videos will be deleted in <b><u>10 mins</u> 🫥 <i></b>(Due to Copyright Issues)</i>.")
        ...
        if not files_:
            ...
            try:
                ...
                title = '@Dankdev  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
                ...
                k = await client.send_message(message.from_user.id,"This Movie File/Video will be deleted in <b><u>10 mins</u> 🫥 <i></b>(Due to Copyright Issues)</i>.",quote=True)
                await asyncio.sleep(600)
                await msg.delete()
        ...
        files = files_[0]
        title = '@Dankdev  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
        ...
        if CUSTOM_FILE_CAPTION:
            try:
                ...
                f_caption=f_caption
        if f_caption is None:
            f_caption = f"@Dankdev  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
        if not await db.has_premium_access(message.from_user.id):
            if not await check_verification(client, message.from_user.id) and VERIFY == True:
                btn = [[
                    InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))

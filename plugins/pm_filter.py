        ...
        try:
            ...
            if not link:
                ...
                if PM_SEARCH == True:
                    ...
                    await message.reply_text(text=f"<b>ʜᴇʏ {user} 😍 ,\n\nʏᴏᴜ ᴄᴀɴ'ᴛ ɢᴇᴛ ᴍᴏᴠɪᴇs ꜰʀᴏᴍ ʜᴇʀᴇ. ʀᴇǫᴜᴇsᴛ ɪᴛ ɪɴ ᴏᴜʀ <a href=https://t.me/Dankdev>ᴍᴏᴠɪᴇ ɢʀᴏᴜᴘ</a> ᴏʀ ᴄʟɪᴄᴋ ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ 👇</b>", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝 ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ", url=f"https://t.me/Dankdev")]]))
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/Dankdev")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        buttons = [[
            ...
            InlineKeyboardButton('Sᴏᴜʀᴄᴇ Cᴏᴅᴇ', url="https://github.com/amitpatle")
        ],[
        ...
        buttons = [[
            InlineKeyboardButton(' Bᴀᴄᴋ', callback_data='help'),
            InlineKeyboardButton('DEV', url="https://github.com/amitpatle")
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    InlineKeyboardButton(" Bᴀᴄᴋ", callback_data="help"),
                    InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/dankdev")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
...
async def advantage_spell_chok(client, name, msg, reply_msg, AP_search):
    mv_id = msg.id
    mv_rqst = name
    ...
    if AI_SPELL_CHECK == True and AP_search == True:
        AP_search_new = False
        AP_ai_msg = await reply_msg.edit_text("<b><i>Advance Ai Try To Find Your Movie With Your Wrong Spelling.</i></b>")
        movienamelist = []
        movienamelist += [movie.get('title') for movie in movies]
        for botnet in movienamelist:
            if mv_rqst.startswith(botnet[0]):
                await auto_filter(client, botnet, msg, reply_msg, AP_search_new)
                break
    else:
        btn = [
            [
